...
- [ ] **원본 corpus의 재배포 권리와 개인정보 검토**
  ...
  - repo에 커밋된 `docs/` 12,630개는 `.gitignore` 추가만으로 히스토리에서
    사라지지 않는다. 비공개 전환이 필요하면 별도 조치가 필요하다
- [ ] Gemini API 키, 서비스 토큰, `SESSION_SECRET`을 실제 secret manager에 등록
- [ ] `python -m scripts.migrate_docs` 로 기존 문서를 데이터 볼륨으로 이관
...
- [ ] 어드민 접근 제한 (리버스 프록시 IP 허용목록 또는 VPN) 과 HTTPS 적용
